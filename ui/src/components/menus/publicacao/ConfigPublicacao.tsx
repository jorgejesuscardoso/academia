...
import { BtnContainer, Container, Delete, Edit, MenuPublicacao } from './style';
import Swal from 'sweetalert2';
import { atualizarPublicacao, deletarPublicacao } from '../../../service/publicacaoApi';
import { deleteLembrete, updateLembrete } from '../../../service/lembrete';
import { deleteEvento, updateEvento } from '../../../service/eventoApi';
...
const ConfigPublicacao = ({ setConfigPub, configPub, item, handleGetFeed, id }: ConfigPublicacaoProps) => {
  ...
  const handleUpdate = async () => {
    ...
    const updatedByUser = GetLocalStorage('user')
    try {
      const data = {
        id: item.id,
        titulo: titles,
        conteudo: content,
        updatedBy: updatedByUser.nome,
      };
    ...
    console.log(data)
      ...
      if (tipo === 'publicação') {      
        ...
        const response = await updateEvento(data);
    
        if (response) {      
          Swal.fire({
            icon: 'success',
            title: 'Sucesso',
            text: 'Evento atualizada com sucesso'
          });
          handleGetFeed();
          setConfigPub(!configPub);
        } 
        ...
        const response = await updateLembrete(data);
    
        if (response) {      
          Swal.fire({
            icon: 'success',
            title: 'Sucesso',
            text: 'Lembrete atualizada com sucesso'
          });
          handleGetFeed();
          setConfigPub(!configPub);
        } 
